...
export const AvatarPicker = ({ form, setForm, inputName, avatarText = "Select your avatar", color = "primary", showText = true, flexDirection = "row" } : AvatarPickerProps) => {
    ...
    return (
        <label htmlFor={inputName} className={"group w-1/2 h-20"}>
            <div className={"relative"}>
                <div className={`avatar flex flex-${flexDirection} justify-center items-center gap-4 opacity-100 group-hover:opacity-0 transition-all ease-in-out absolute`}>
                    {showText && <span className={`text-${color}`}>{avatarText}</span>}
                    <div className="w-20 mask mask-squircle group-hover:opacity-50 transition-all ease-in">
                        <img src={image} />
                    </div>
                </div>
                <div className={`avatar placeholder flex flex-${flexDirection} justify-center items-center gap-4 opacity-0 group-hover:opacity-100 transition-all ease-in-out absolute`}>
                    {showText && <span className={`text-${color}`}>{avatarText}</span>}
